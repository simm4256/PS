...
타짜_LV2.cpp
알고리즘 : 완전탐색
교체할 2장의 카드를 선택하고, 해당 카드 위치에 36종류의 카드를 대입하면서 가장 우위의 조합을 찾아냅니다.
경우의 수는 5C2 * 36 * 36 = 12960 개 입니다.

문제에 트릭이 숨어있는데, 정답으로 FH와 TP는 존재할 수 없습니다.
입력으로 주어진 카드 중 숫자가 같은 카드가 있으면 나머지 카드 중 2장을 바꿔 FC를 만들 수 있고,
모든 숫자가 다르다면 FH를 만들 수 없음이 자명하므로 FH는 어떤 입력에서도 답이 될 수 없습니다.
그리고 모든 숫자가 다를 때 ST를 만들 수 없는 경우는 없으므로 TP는 답이 될 수 없습니다.
이 트릭을 눈치챘다면 구현 문제인 만큼 코드양이 줄어 테스트 시간을 절약할 수 있을 것입니다.
*/
